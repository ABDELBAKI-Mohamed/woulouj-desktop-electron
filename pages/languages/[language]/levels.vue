...
const { language } = useRoute().params as Record<"language", string>;
const { getLevels, getFlag } = useLanguagesStore();
</script>
  ...
  <section class="w-full h-full flex">
    <div class="w-full h-full flex justify-center flex-col items-center">
      ...
      <div class="w-fit grid gap-2 grid-cols-3">
        <NuxtLink
          v-for="(level, index) in getLevels(language)"
          :to="
            getRightPath(useRoute().path, level.concat('/').concat('modules'))
          "
          :key="index"
          class="h-44 w-48"
        >
