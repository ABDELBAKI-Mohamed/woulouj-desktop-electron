...
const { getAgesFlagsAndPath } = useLanguagesStore();
</script>
  ...
  <section class="w-full h-full flex">
    <div class="w-full h-full flex justify-center flex-col items-center">
      ...
      <div class="w-fit grid gap-2 grid-cols-3">
        <NuxtLink
          v-for="(age, index) of getAgesFlagsAndPath()"
          :to="
            getRightPath(useRoute().path, age.path.concat('/').concat('groups'))
          "
          :key="index"
          class="h-44 w-48"
        >
          ...
          >
            <img
              class="absolute top-0 left-0 w-full opacity-20 group-hover:opacity-60 h-full object-cover transition-all duration-250"
              :src="age.img"
              alt=""
            />
            <span
