...
import { newTeacherT } from "~~/types";

const { toggleModal } = useGlobalState();
const { toggleToast } = useToast();
const { createTeacher, TeacherFeilds, TeacherPlaceHolders } = useTeacher();

const teacher = reactive<newTeacherT>({
  firstname: "",
  lastname: "",
  fullname: "",
});

const createAteacher = async () => {
  try {
...
<template>
  ...
  >
    ...
    >
      <span class="col-span-4 col-start-2"> create a teacher </span>
      <span
    ...
    <div class="h-full w-full flex flex-col gap-2">
      <ImageUploader @on:save="(value) => (teacher.image = value)" />
      <UiInput
        v-for="(feild, index) of TeacherFeilds"
        :Placeholder="TeacherPlaceHolders[index]"
        @on:input="(text:any)=> teacher[feild] = text"
      />
      <UiButton @on:click="createAteacher" :Disable="false" colorTheme="a">
        add teacher
      </UiButton>
    </div>
  </div>
